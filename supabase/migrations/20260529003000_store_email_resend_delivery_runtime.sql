-- Store email Resend delivery runtime.
-- Additive only: preserves queued email logs and existing settings.
...
create index if not exists email_event_logs_pending_delivery_idx
on public.email_event_logs(workspace_id, store_id, created_at)
where status = 'pending' and resend_message_id is null;

create index if not exists email_event_logs_resend_message_idx
on public.email_event_logs(resend_message_id)
