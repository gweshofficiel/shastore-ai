...
import type { AdminMarketplaceControl } from "@/lib/admin/data";
import { loadMarketplaceControlSafe } from "@/lib/admin/marketplace-loader";
import {
...
function MarketplaceRuntimeRecoveryNotice({ message }: { message: string }) {
  return (
    <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5">
      <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Marketplace runtime recovery</p>
      <p className="mt-2 text-sm font-semibold text-amber-900">
        Marketplace data could not be loaded. The admin shell is still available; retry after checking runtime logs.
      </p>
...
function MarketplaceAdminPanelContent({ control }: { control: AdminMarketplaceControl }) {
  return (
    <>
      <AdminStatGrid
        stats={[
...
export function MarketplaceAdminPanel() {
  const [state, setState] = useState<MarketplacePanelState>({ status: "loading" });

  useEffect(() => {
    let cancelled = false;

    void loadMarketplaceControlSafe().then((result) => {
      if (cancelled) {
        return;
      }
  ...
  if (state.status === "loading") {
    return (
      <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5">
        <p className="text-sm font-semibold text-slate-600">Loading marketplace runtime data…</p>
      </div>
    );
  }
