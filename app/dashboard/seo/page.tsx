      ...
      <Card className="p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-muted">Manage redirects, Search Console, Meta Pixel, and GA4 separately from sitemap and robots.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              className="rounded-full bg-slate-100 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-muted"
              href={`/dashboard/seo/google-analytics${activeStore ? `?storeId=${encodeURIComponent(activeStore.id)}` : ""}`}
            >
              Google Analytics
            </Link>
            <Link
