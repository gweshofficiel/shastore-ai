...
import { createPayPalPlatformCheckout } from "@/lib/billing/paypal-platform";
import {
...
export async function POST(request: Request) {
  ...
  console.info("[paypal_checkout_request_started]", {
    planId: upgrade.planId,
    userId: user.id
  });

  const checkout = await createPayPalPlatformCheckout({
  ...
  console.info("[paypal_checkout_redirect_ready]", {
    planId: upgrade.planId,
    url: checkout.url,
    userId: user.id
  });
