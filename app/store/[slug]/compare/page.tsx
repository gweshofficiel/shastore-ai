...
import { ComparePageClient } from "@/components/storefront/product-compare";
...
export async function generateMetadata({
  ...
  if (!preview) {
    return {
      title: "Compare not found | SHASTORE AI",
      robots: { follow: false, index: false }
    };
  }

  return {
    title: `Compare products | ${preview.store.title}`,
    description: `Compare products from ${preview.store.title}.`
  };
}

export default async function StoreComparePage({ params }: ComparePageProps) {
  const { slug } = await params;
  const preview = await getPublicStorefrontPreview(slug);
      ...
      <main className="min-h-screen bg-slate-50 px-4 py-16 text-ink sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl rounded-[2rem] border border-dashed border-slate-300 bg-white p-10 text-center">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-[-0.04em] text-ink">
            This compare page is not available.
          </h1>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-muted">
            Compare up to four products from this store. Your compare list is saved on this browser only.
          </p>
        </div>
