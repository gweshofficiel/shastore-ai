...
function returnMessage(status: string | undefined) {
  const messages: Record<string, string> = {
    duplicate: "A return request already exists for this order.",
    failed: "Return request could not be submitted. Please try again.",
...
async function loadCustomerReturnRequests({
  admin,
  orderId,
  phone,
  source,
...
async function loadTrackedOrder({
  ...
  if (order) {
    ...
    const returnRequests = await loadCustomerReturnRequests({
      admin,
      orderId: order.id,
...
function ReturnRequestForm({
  ...
  return (
    <form action={requestOrderReturnAction} className="mt-4 grid gap-3">
      ...
      <label className="grid gap-2 text-sm font-semibold text-ink">
        ...
        <textarea
          ...
          name="notes"
          placeholder="Tell the seller what you want to return and why."
        />
      </label>
